...
import { DEFAULT_COUNTRY, DEFAULT_CURRENCY } from "./localeContext";
...
/**
 * Fetch the user's locale, associated currency, and country. This is done based on the user
...
export default async function getLocale(): Promise<Locale> {
  ...
  if (!locale) {
    ...
    if (!resp.ok) {
      ...
      currency = (getCurrency(locale) as Currency) || DEFAULT_CURRENCY;
    } else {
